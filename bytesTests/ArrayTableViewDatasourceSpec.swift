    ...
    override func spec() {
        ...
        describe("numberOfSections") {
            it("should always return 1") {
                let dataStub = ArrayTableViewDatasourceDataStub()
                let datasource = ArrayTableViewDatasource(data: dataStub.data, createCell: dataStub.createCell)
                expect(datasource.numberOfSections(in: dataStub.tableView)).to(equal(1))
            }
        }
        
        describe("numberOfRowsInSection") {
            it("should return the size of the array for section 0") {
                let dataStub = ArrayTableViewDatasourceDataStub()
                let datasource = ArrayTableViewDatasource(data: dataStub.data, createCell: dataStub.createCell)
                expect(datasource.tableView(dataStub.tableView, numberOfRowsInSection: 0)).to(equal(dataStub.data.count))
            }
            it("should return 0 for every section > 0") {
                let dataStub = ArrayTableViewDatasourceDataStub()
                let datasource = ArrayTableViewDatasource(data: dataStub.data, createCell: dataStub.createCell)
                expect(datasource.tableView(dataStub.tableView, numberOfRowsInSection: 1)).to(equal(0))
            }
        }
